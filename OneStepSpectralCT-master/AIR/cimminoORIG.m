...
%
% Computes Cimmino's projection method for the linear system Ax = b:
%
%       x^{k+1} = x^k + lambda_k*A^T*M*(b-A*x^k)
%
% where M = (1/m)*diag(w_i/||a^i||_2^2) and w_i are weights (detault w_i = 1).
%
% Input:
%   A          m times n matrix.
...
%               diagonal of the matrix M = 1/m*diag(1/||a^i||^2_2).
%
% See also: landweber, cav, drop, sart.

% Maria Saxild-Hansen and Per Chr. Hansen, June 11, 2011, DTU Informatics.

% Reference: C. D. Meyer, Matrix Analysis and Applied Linear Algebra,
% SIAM, Philadelphia, 2000 (this reference uses reflections and there
% includes a redundant factor of 2).
...
while ~stop
    ...
    if casel == 1
        % Using constant value of lambda.
