...
T1 = readmatrix('NISTequivalentofRealData_LargerDict_MatrixTXray128chan_42_MinEn20.35_MaxEn161.15_mat_CORRECTED.csv');
T1 = 0.015*T1;
T1(43,:) = mean(recsParticles(:,130:140,130:140),[2,3]); %quartz

T0 = zeros(3,128);
T0(1,:) = recsParticles(:,24,155); %gold
T0(2,:) = recsParticles(:,109,122); %thungsten
T0(3,:) = recsParticles(:,207,106); %lead
T0(4,:) = mean(recsParticles(:,130:140,130:140),[2,3]); %quartz

%% true dictionaries

T_truef = zeros(4,128);
T_truef(1,:) = readmatrix([globalDataDir 'GoldSpectrum.csv']); %gold
T_truef(2,:) = recsParticles(:,109,122); %thungsten
T_truef(3,:) = readmatrix([globalDataDir 'LeadSpectrum.csv']); %lead
T_truef(4,:) = mean(recsParticles(:,130:140,130:140),[2,3]); %quartz
