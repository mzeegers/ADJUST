function [x,f,funEvals,projects] = minConf_SPGQ(funObj,x,funProj,options)
...
if testOpt
    projects = projects+1;
    if max(abs(funProj(x-g,rId)-x)) < optTol
        if verbose >= 1
...
while funEvals <= 10
    ...
    while f_new > funRef + suffDec*g'*(x_new-x) || ~isLegal(f_new)
        ...
        t_prev = temp;
        if curvilinear
            x_new = funProj(x + t*d,rId);
            projects = projects+1;
        else
            x_new = x + t*d;
        end
        [f_new,g_new] = funObj(x_new,rId);
        funEvals = funEvals+1;
    ...
    if testOpt
        optCond = max(abs(funProj(x-g,rId)-x));
        projects = projects+1;
    end
...
if verbose >= 1
    fprintf('%10d %10d %10d %15.5e\n',i,funEvals*funEvalMultiplier,projects,f);
end
